...
public class FallingExampleStaticDynamic {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < n; i++){
			try {
				overshoot += simulator.simulateBlockedOvershootCosts(problem, solution);
			} catch (ConvolutionNotDefinedException e) {
				System.out.println(e);
			} catch (SolvingInitialisiationException e) {
				System.out.println(e);
		...
		System.out.println("Average simulated blind stock costs: "+overshoot/n);
		try {
			System.out.println("Calculated expected blind stock costs: "+simulator.simulateBlockedOvershootCosts(problem, solution));
		} catch (ConvolutionNotDefinedException e) {
			System.out.println(e);
		} catch (SolvingInitialisiationException e) {
			System.out.println(e);
